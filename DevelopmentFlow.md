# 開発フロー
**GitHub の Pull Request を用いた GitHub flow で行う**
...
## 開発フロー概要
1. [ 設計者/不具合報告者 ] Issue(チケット)を起票する。
   - ※Github の場合、Labels には、[enhancement], [bug]などを、JIRAの場合、[Story], [Bug]を指定する。
...
1. ビルドが完了し配布された後に
1. [ 開発者 ] Issue(チケット) をクローズ(Resolve)する。
